...
```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#FFFFFF', 'primaryBorderColor': '#232F3E', 'primaryTextColor': '#000000', 'lineColor': '#232F3E', 'fontSize': '13px', 'nodeBorder': '#232F3E', 'mainBkg': '#FFFFFF', 'clusterBkg': '#F5F5F5', 'clusterBorder': '#232F3E', 'titleColor': '#000000', 'edgeLabelBackground': '#FFFFFF', 'nodeTextColor': '#000000'}}}%%
flowchart TB
    subgraph HC["Health Insurer — Clinical Data Pipeline (AWS HealthLake repo)"]
        direction TB
        STEP1["[1] Synthetic Clinical Data Generation<br/>Synthea FHIR R4"]
        STEP2["[2] FHIR Data Ingestion<br/>AWS HealthLake<br/>ICD-10-CM, RxNorm, LOINC"]
        STEP3["[3] Feature Engineering<br/>Apache Iceberg on Amazon S3 + Amazon Athena SQL<br/>Patient-level ADR features"]
        STEP4["[4] De-identification<br/>Amazon Comprehend Medical PHI Detection<br/>(skipped for synthetic data)"]:::dashed
        S3H["[5] Amazon S3<br/>Enriched Clinical Feature Table"]
        GLUEH["[6] AWS Glue Data Catalog<br/>Clinical Feature Table"]
        STEP1 --> STEP2
        STEP2 --> STEP3
        STEP3 --> STEP4
        STEP4 --> S3H
    ...
    subgraph PHARMA["Pharma Company — Drug Exposure Pipeline (AWS Clean Rooms repo)"]
        direction TB
        LAMBDA["[7] Synthetic Drug Exposure Data Generation<br/>AWS Lambda<br/>dosage, duration, safety scores, black-box warnings"]
        S3P["[8] Amazon S3<br/>Drug Exposure Data"]
        GLUEP["[9] AWS Glue Data Catalog<br/>Drug Exposure Data"]
        LAMBDA --> S3P
        S3P --> GLUEP
    end

    subgraph UC2["AWS Clean Rooms ML Collaboration (AWS Clean Rooms repo)"]
        direction TB
        CT["[10] Configured Tables + Analysis Rules<br/>AWS Clean Rooms"]
        CRML["[11] AWS Clean Rooms ML"]
        TRAIN["[12] Model Training Job<br/>GradientBoosting / XGBoost"]
        INFER["[13] Inference / Scoring Job"]
        CT -->|"JOIN on patient_id"| CRML
        CRML --> TRAIN
        CRML --> INFER
        TRAIN -->|"trained model"| INFER
    end

    subgraph ECR["Amazon ECR — Container Images (AWS Clean Rooms repo)"]
        direction LR
        TI["[14] Training Container Image<br/>GradientBoosting + sklearn"]
        II["[15] Inference Container Image<br/>SageMaker PyTorch base"]
    end
    ...
    subgraph OUT["Analysis Output"]
        direction TB
        S3O["[16] Amazon S3<br/>ADR Propensity Scores"]
        QS["[17] Amazon QuickSight Dashboards<br/>Technical and business metrics"]
    end

    INFER --> S3O
    S3O --> QS

    classDef dashed stroke-dasharray: 5 5, stroke: #888888
```
...
**Boxes [5] and [6]: Feature Table Storage and Registration**: The computed feature table is written to Amazon S3 in Parquet format and registered in the AWS Glue Data Catalog. This makes it queryable and ready to be contributed as a configured table to the AWS Clean Rooms collaboration.
...
**Boxes [11], [12], [13]: AWS Clean Rooms ML Training and Inference** *(AWS Clean Rooms repo)*: AWS Clean Rooms ML joins both tables on the shared patient token, trains a GradientBoosting model on the combined signal, and runs inference to produce an ADR propensity score for every matched patient. The training and inference containers are pulled from Amazon ECR (boxes [14] and [15]).

**Boxes [16] and [17]: Results and Dashboards** *(AWS Clean Rooms repo)*: Scores are written to Amazon S3 and visualized in the Amazon QuickSight dashboards described in [section 3](#3-example-dashboards).
...
**[6] Feature Table Registration (AWS Glue Data Catalog)**
...
> The AWS Clean Rooms ML pharma dataset is generated with a target of approximately 50,000 patients. The AWS Clean Rooms ML join quality, and therefore the significance of the ADR risk scores and dashboard metrics, depends on the number of patients that match across both datasets. In the context Deploying the large stack with 50,000 patients maximizes the overlap with the AWS Clean Rooms ML dataset and produces the most meaningful analysis results. Using a higher patient count will make the deployment longer without adding statistical significance, because the matching population is bounded by the AWS Clean Rooms ML dataset size. Using a lower count will reduce the number of matched patients and lower the confidence of the model scores.
...
Step 1, 2, 3 and 5 are common to both Cloudformation Template.
Step 4a explains how to deploy small patients dataset template.
Step 4b explains how to deploy large patients dataset template.
...
> **SM2 is independently re-runnable.** If SM2 fails for any reason, you can re-start it directly from the AWS Step Functions console without re-running AWS CodeBuild or the FHIR import. The FHIR data in AWS HealthLake is preserved.
